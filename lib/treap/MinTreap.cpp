...
template<typename key_t, typename priority_t>
using MinTreap = TreapNode<key_t, priority_t>*;

template<typename key_t, typename priority_t>
auto Cnt(MinTreap<key_t, priority_t> treap) {
    if (treap) {
        return treap->cnt;
    } else {
        return 0;
...
template<typename key_t, typename priority_t>
auto UpdateCnt(MinTreap<key_t, priority_t> treap) {
    if (treap) {
...
template<typename key_t, typename priority_t>
auto Split(MinTreap<key_t, priority_t> treap, key_t key,
           MinTreap<key_t, priority_t>& out_lhs,
           MinTreap<key_t, priority_t>& out_rhs) {
    if (!treap) {
        out_lhs = nullptr;
        out_rhs = nullptr;
...
template<typename key_t, typename priority_t>
auto Insert(MinTreap<key_t, priority_t>& treap,
...
template<typename key_t, typename priority_t>
auto Merge(MinTreap<key_t, priority_t>& out,
           MinTreap<key_t, priority_t> lhs,
           MinTreap<key_t, priority_t> rhs) {
    if (!lhs) {
        out = rhs;
    } else if (!rhs) {
        out = lhs;
...
template<typename key_t, typename priority_t>
auto Erase(MinTreap<key_t, priority_t>& treap, key_t key) {
    if (treap->key == key) {
